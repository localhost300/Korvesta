...
export function Footer() {
  return <footer className="mt-16 border-t" style={{ borderColor: "var(--border-soft)" }}><div className="container-shell grid gap-10 py-12 md:grid-cols-[1.25fr_4fr]"><div><Logo/><p className="mt-4 max-w-[240px] text-sm leading-6 text-muted">Market data, education and tools for informed decision-making.</p><p className="mt-4 max-w-[260px] text-[11px] leading-5 text-muted">Pre-launch product. No real trading, custody or investment execution is available.</p></div><div className="grid grid-cols-2 gap-8 sm:grid-cols-4">{groups.map((group) => <div key={group.title}><h3 className="mb-3 text-sm font-semibold">{group.title}</h3><div className="grid gap-2.5 text-xs text-muted">{group.links.map(([label, href]) => <Link key={label} href={href} className="hover:text-[var(--amber)]">{label}</Link>)}</div></div>)}</div></div><div className="container-shell flex flex-col gap-3 border-t py-5 text-xs text-muted sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: "var(--border-soft)" }}><span>© 2026 Korvesta. All rights reserved.</span><div className="flex gap-6"><Link href="/legal/privacy">Privacy Policy</Link><Link href="/legal/terms">Terms of Use</Link><Link href="/legal/cookies">Cookie Policy</Link></div></div></footer>;
}
